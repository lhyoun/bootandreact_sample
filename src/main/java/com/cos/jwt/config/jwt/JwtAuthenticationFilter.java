		...
		if(!method.equals("POST")) {
			...
			try {
				...
				if(personEntity == null) {
					...
					String jwtToken = 
							JWT.create()
							.withSubject("토큰제목")	// subject가 key
							.withExpiresAt(new Date(System.currentTimeMillis()+1000*60*30))	// 위 두개는 요청하지 않아도 미리 등록된 claim. 그래서 key값을 입력하지 않아도 됨
							.withClaim("id", personEntity.getId())
							.withClaim("username", personEntity.getUsername())	// 없어도 무방
